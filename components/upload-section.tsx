...
export function UploadSection({ onUpload }: UploadSectionProps) {
  ...
  return (
    <Card className="border-0 shadow-lg bg-white dark:bg-slate-800">
      <CardHeader className="bg-gradient-to-r from-teal-700 to-blue-800 text-white rounded-t-lg">
      ...
      <CardContent className="p-6">
        <Tabs defaultValue="video" value={activeTab} onValueChange={setActiveTab} className="w-full">
          <TabsList className="grid grid-cols-3 mb-6">
            <TabsTrigger value="video" className="data-[state=active]:bg-teal-600 data-[state=active]:text-white">
              <FileVideo className="mr-2 h-4 w-4" />
              Video
            </TabsTrigger>
            <TabsTrigger value="audio" className="data-[state=active]:bg-teal-600 data-[state=active]:text-white">
              <FileAudio className="mr-2 h-4 w-4" />
              Audio
            </TabsTrigger>
            <TabsTrigger value="text" className="data-[state=active]:bg-teal-600 data-[state=active]:text-white">
            ...
            {!selectedFile ? (
              <div
                className={cn(
                  ...
                  dragActive ? "border-teal-600 bg-teal-50" : "border-gray-300",
                )}
                ...
                onDrop={handleDrop}
              >
                <FileVideo className="h-12 w-12 text-teal-600 mb-4" />
              ...
              <div className="flex items-center justify-between p-4 border rounded-lg">
                <div className="flex items-center">
                  <FileVideo className="h-8 w-8 text-purple-600 mr-3" />
                  <div>
          ...
          <TabsContent value="audio" className="mt-0">
            {!selectedFile ? (
              <div
                ...
                onDrop={handleDrop}
              >
                <FileAudio className="h-12 w-12 text-purple-600 mb-4" />
                <p className="text-center mb-4">
              ...
              <div className="flex items-center justify-between p-4 border rounded-lg">
                <div className="flex items-center">
                  <FileAudio className="h-8 w-8 text-purple-600 mr-3" />
                  <div>
                    <p className="font-medium">{selectedFile.name}</p>
                    <p className="text-sm text-muted-foreground">{(selectedFile.size / (1024 * 1024)).toFixed(2)} MB</p>
                  </div>
